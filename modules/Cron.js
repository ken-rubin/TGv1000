...
/* TOXXX DONE											var strQuery = "select distinct u.userName, u.firstName from " + self.dbname + "user u inner join " + self.dbname + "projects p on u.id = p.ownedByUserId where p.comicProjectId=" + classIth.baseProjectId + " and p.isClass=0;";*/
            											var strQuery = "select distinct u.userName, u.firstName from " + self.dbname + "user u inner join " + self.dbname + "projects p on u.id = p.ownedByUserId where p.id in (" + self.dbname + "getProjectsLinkedToGivenProjectByComic(" + classIth.baseProjectId + ")) and p.isClass=0;";
														sql.execute(strQuery,
															function(rows) {
