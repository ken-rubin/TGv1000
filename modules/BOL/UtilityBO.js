...
/* TOXXX DONE           var strQuery = "select u.*, ug.name as usergroupName from " + self.dbname + "user u inner join " + self.dbname + "usergroups ug on u.usergroupId=ug.id where u.id in (select ownedByUserId from " + self.dbname + "projects where id<>comicProjectId and comicProjectId=" + req.body.projectId + ");"; */
    ...
    self.routeSearchResources = function (req, res) {
        ...
        try {
            ...
            strQuery = "create table " + self.dbname + tempTableUniqueName + " (resourceId INT UNSIGNED, sindex DOUBLE) ENGINE=InnoDB;";
            // TODO: Add public=1 after we start setting it.
            strQuery += "insert " + tempTableUniqueName + " select id, soundex_match_all(" + mysql.escape(searchPhrase) + ", description, ' ') from " + self.dbname + "resources where length(description)>0 and resourceTypeId=" + req.body.resourceTypeId + ";";
            strQuery += "select r.*, t.sindex from " + self.dbname + "resources r inner join " + tempTableUniqueName + " t on t.resourceId=r.id where r.id in (select resourceId from " + tempTableUniqueName + " where sindex>0);";
