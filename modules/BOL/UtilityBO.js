                ...
                [
                    ...
                    function(passOn, cb) {
                        ...
                        if (req.body.privilegedUser === "1") {
                            strQuery = "select p.id, p.name, p.description, p.imageId as projectImageId from " + self.dbname + "projects p where p.isCoreProject=1;";
                        } else {
                            strQuery = "select p.id, p.name, p.description, p.imageId as projectImageId from " + self.dbname + "projects p where p.isCoreProject=-1;";   // we want this to return no rows but use [0].
                        }

                        if (passOn.idString.length) {
                            // Owned by user. Same for both priv and non-priv.
                            strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId, p.comicProjectId from " + self.dbname + "projects p where p.ownedByUserId=" + req.user.userId + " and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";

                            // Others' accounts
                            if (req.body.privilegedUser === "1") {
                                // A privileged user doesn't care about public/private.
                                strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId from " + self.dbname + "projects p where p.ownedByUserId<>" + req.user.userId + " and p.isCoreProject=0 and p.isProduct=0 and p.isClass=0 and p.isOnlineClass=0 and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";
                            } else {
                                // A non-privileged user can retrieve only public projects and only "normal" projects.
                                strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId from " + self.dbname + "projects p where p.ownedByUserId<>" + req.user.userId + " and p.public=1 and p.isCoreProject=0 and p.isProduct=0 and p.isClass=0 and p.isOnlineClass=0 and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";
                            }

                            // Products
                            if (req.body.privilegedUser === "1") {
                                // A privileged user doesn't care about active.
                                strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId, pr.level, pr.difficulty, pr.productDescription, pr.imageId as prImageId, pr.price, pr.active, pr.videoURL from " + self.dbname + "projects p inner join " + self.dbname + "products pr on pr.baseProjectId=p.id where p.isProduct=1 and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";
                            } else {
                                // A non-privileged user just sees active projects.
                                strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId, p.isProduct, pr.* from " + self.dbname + "projects p inner join " + self.dbname + "products pr on pr.baseProjectId=p.id where pr.active=1 and p.isProduct=1 and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";
                            }

                            // Classes
                            if (req.body.privilegedUser === "1") {
                                // A privileged user doesn't care about active.
                                strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId, cl.level, cl.difficulty, cl.classDescription, cl.imageId as clImageId, cl.price, cl.schedule, cl.active, cl.classNotes, cl.zip, cl.maxClassSize from " + self.dbname + "projects p inner join " + self.dbname + "classes cl on cl.baseProjectId=p.id where p.isClass=1 and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";
                            } else {
                                // A non-privileged user just sees active classes.
                                strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId, p.isClass cl.* from " + self.dbname + "projects p inner join " + self.dbname + "classes cl on cl.baseProjectId=p.id where cl.active=1 and p.isClass=1 and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";
                            }
                            ...
                            if (req.body.privilegedUser === "1") {
                                ...
                                // A non-privileged user just sees active classes.
                                strQuery += "select distinct p.id, p.name, p.description, p.imageId as projectImageId, p.isOnlineClass cl.* from " + self.dbname + "projects p inner join " + self.dbname + "onlineclasses cl on cl.baseProjectId=p.id where cl.active=1 and p.isOnlineClass=1 and p.id in (select distinct projectId from " + self.dbname + "project_tags pt where " + passOn.idCount + "=(select count(*) from " + self.dbname + "project_tags pt2 where pt2.projectId=pt.projectId and tagId in (" + passOn.idString + ")));";
                            }
                        }
