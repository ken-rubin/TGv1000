...
define(["Core/snippetHelper", "Core/errorHelper", "Core/resourceHelper", "Core/ScrollRegion"], 
	function (snippetHelper, errorHelper, resourceHelper, ScrollRegion) {
		...
		try {
			...
			// Define the TypeSearchDialog constructor function.
			var functionTypeSearchDialog = function () {
				...
				try {
						...
						try {
							...
							$.ajax({
								...
								data: { 
									...
									templateFile: "Dialogs/TypeSearchDialog/typeSearchDialog"
								}, 
								dataType: "HTML",
								method: "POST",
					...
					self.callFunctionOK = function(iTypeId) {

						try {

							m_functionOK(iTypeId);
							m_dialog.close();
						...
						try {
							...
							var exceptionRet = m_scISImageStrip.create(
								...
								function(){
						    		...
						    		var typeId = m_searchResultRawArray[j].id;
						    		self.callFunctionOK(typeId);
						    	});
							if (exceptionRet) {
					...
					var m_functionSearchBtnClicked = function () {
					    ...
					    try {
					        ...
					        // var posting = $.post("/BOL/UtilityBO/SearchTypes", 
					        // 	{
					        // 		tags: tags, 
					        // 		userId: g_strUserId,
